...
export default function ThreatActors() {
  ...
  useEffect(() => { loadEntity<ThreatActor[]>('threat_actors.json').then(setActors).finally(() => setLoading(false)) }, [])
    ...
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[10px] text-muted-foreground mt-0.5">Public domain attribution verileri. Eğitim amaçlıdır.</p>
        </div>
        <span className="text-[10px] text-muted-foreground">{actors.length} aktör · {motivations.length} motivasyon</span>
      </div>

      <div className="flex items-center gap-1.5 flex-wrap">
        <Filter className="w-3 h-3 text-muted-foreground" />
        <button onClick={()=>setMotFilter('all')} className={`px-2 py-1 rounded-md text-[10px] border transition-colors ${motFilter==='all'?'bg-primary/10 border-primary/30 text-primary':'border-border text-muted-foreground hover:bg-accent'}`}>Tümü</button>
        {motivations.map(m=><button key={m} onClick={()=>setMotFilter(m)} className={`px-2 py-1 rounded-md text-[10px] border transition-colors ${motFilter===m?'bg-primary/10 border-primary/30 text-primary':'border-border text-muted-foreground hover:bg-accent'}`}>{MOTIVATION_LABELS[m]??m}</button>)}
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        {filtered.map(a=>{
          const Icon = (Icons as unknown as Record<string, React.ComponentType<{className?:string}>>)[a.glyph.icon] ?? Shield
            ...
            <Link key={a.id} to={`/threat-actors/${a.id}`} className="flex flex-col p-4 rounded-lg border border-border bg-card hover:border-primary/20 hover:bg-accent transition-all group">
              <div className="flex items-start gap-3 mb-2">
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-[10px] text-muted-foreground truncate">{a.aliases.slice(0,2).join(', ')}</div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-1 flex-wrap">
                {a.motivation.slice(0,2).map(m=><span key={m} className="text-[9px] bg-muted px-1 rounded text-muted-foreground">{MOTIVATION_LABELS[m]??m}</span>)}
                {a.motivation.length>2 && <span className="text-[9px] text-muted-foreground">+{a.motivation.length-2}</span>}
              </div>
              {matchCount>0 && <div className="mt-auto pt-2 text-[10px] text-primary/70">{matchCount} incident eşleşmesi</div>}
            </Link>
          )
        })}
      </div>
