...
export default function Endpoints() {
  ...
  const handleIsolate = (hostname: string) => toast.success(`${hostname} izole edildi.`, { description: 'Kurgusal EDR aksiyonu.', duration: 2500 })
  const handleRestore = (hostname: string) => toast(`${hostname} izolasyonu kaldırıldı.`, { description: 'Kurgusal EDR aksiyonu.', duration: 2500 })
    ...
    <div className="p-6 space-y-4">
      ...
      <div className="flex items-center gap-1.5 flex-wrap">
        ...
        {(['all', 'workstation', 'laptop', 'server', 'mobile'] as const).map((t) => (
          <button key={t} onClick={() => setTypeFilter(t)} className={`px-2 py-1 rounded-md text-[10px] border transition-colors ${typeFilter === t ? 'bg-primary/10 border-primary/30 text-primary font-medium' : 'border-border text-muted-foreground hover:bg-accent'}`} aria-pressed={typeFilter === t}>
                  ...
                  <tr key={ast.asset_id} className="border-b border-border last:border-0 hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-3 py-2">
                      {ast.isolation_status === 'isolated' ? (
                        <span className="inline-flex items-center gap-1 text-severity-critical text-[10px] font-medium"><WifiOff className="w-3 h-3" />İzole</span>
                      ) : (
                        <span className="inline-flex items-center gap-1 text-severity-low text-[10px]"><Wifi className="w-3 h-3" />Açık</span>
                      )}
                    </td>
                    <td className="px-3 py-2">
                      {ast.isolation_status === 'isolated' ? (
                        <button onClick={() => handleRestore(ast.hostname)} className="inline-flex items-center gap-1 text-[9px] px-1.5 py-0.5 rounded border border-border hover:bg-accent transition-colors" aria-label="İzolasyonu kaldır">
                          <ShieldCheck className="w-3 h-3" />Kaldır
                        </button>
                      ) : (
                        <button onClick={() => handleIsolate(ast.hostname)} className="inline-flex items-center gap-1 text-[9px] px-1.5 py-0.5 rounded border border-severity-critical/30 text-severity-critical hover:bg-severity-critical/10 transition-colors" aria-label="Cihazı izole et">
